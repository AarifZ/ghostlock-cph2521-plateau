...
/* 6.12 default; 5.10 ports (Quest IonStack / CPH2521) use -0xe20 via -D */
...
/* Overlay window must outlast consumer punch (PSELECT_ROUTE_DELAY_USEC).
 * Was 200ms; EXP_F delay=500 returned success=0 because select timed out first.
 * 1.5s leaves headroom for 50–150ms (plateau floor) and longer digs. */
...
int iouring_ring_reclaim_one(unsigned char *payload, size_t plen, int do_register);
