 ...
 * KIMAGE_TEXT_BASE is the link address of _text (unslid). Override per-device
 * with -DKIMAGE_TEXT_BASE=... (CPH2521 5.10 uses 0xffffffc008000000). */
...
#endif
/* physmap — override per-SoC via -D if needed (Quest 5.10 uses END=0xffffff9000000000) */
...
/* Struct field offsets — 6.12 defaults; 5.10 overrides via -D / GHOSTLOCK_KERNEL_5_10 */
 ...
 *
 * 6.12 GKI (Ace 6T / default): llseek sits at +0x10 (extra early field).
 * Android 5.10 (CPH2521 boot.img ashmem_fops / configfs_bin_file_operations):
 *   classic layout + mmap_supported_flags after mmap → open at +0x70, etc.
 * Verified by dumping real fops tables from CPH2521_16.0.5.1002 Image.
 */
...
/* unsigned long mmap_supported_flags at +0x68 */
