...
for n in $(seq 1 $MAX); do
  ...
  R=$(MSYS_NO_PATHCONV=1 "$ADB" -s $WIFI shell "grep -a 'DOUBLE\|CRED LANDED\|uid0_WIN\|ROOTED' /data/local/tmp/$L.txt 2>/dev/null | tail -4; cat /data/local/tmp/ROOTED_ID /data/local/tmp/ROOTED 2>/dev/null; cut -d. -f1 /proc/uptime" 2>/dev/null | tr -d '\r')
